...
if (! function_exists('convert_price_api')) {
    function convert_price_api($price)
    {
        $business_settings = BusinessSetting::where('type', 'system_default_currency')->first();
        if($business_settings != null){
            $currency = Currency::find($business_settings->value);
...
//formats currency
if (! function_exists('format_price_api')) {
    function format_price_api($price)
    {
        ...
        if(BusinessSetting::where('type', 'symbol_format')->first()->value == 1){
            return currency_symbol_api().$fomated_price;
        }
        return $fomated_price.currency_symbol_api();
    }
}
...
if (! function_exists('single_price_api')) {
    function single_price_api($price)
    {
        return format_price_api(convert_price_api($price));
    }
}
...
if (! function_exists('currency_symbol_api')) {
    function currency_symbol_api()
    {
        $code = \App\Currency::findOrFail(\App\BusinessSetting::where('type', 'system_default_currency')->first()->value)->code;
            $header =  request()->header('currency');
            if($header != null){
...
if (!function_exists('uploaded_asset_nullable')) {
    function uploaded_asset_nullable($id)
    {
        if ($id) {
            $ids = explode(',',$id);
            if (($asset = Upload::find($ids[0])) != null) {
...
if (!function_exists('uploaded_assets_nullable')) {
    function uploaded_assets_nullable($id)
    {
        $photos = array();
        if ($id) {
            $ids = explode(',',$id);
            foreach ($ids as $id){
                if (($asset = Upload::find($ids[0])) != null) {
                    array_push($photos,my_asset($asset->file_name));
                }
            }
        }else {
