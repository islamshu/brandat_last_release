        ...
        <table class="table">
            ...
            <tbody>
                @php
                    $subtotal = 0;
                    $tax = 0;
                    $shipping = App\BusinessSetting::where('type','shipping_cost')->first()->value;
                @endphp
                @foreach (Session::get('cart')->where('owner_id', Session::get('owner_id')) as $key => $cartItem)
                    @php
                        $product = \App\Product::find($cartItem['id']);
                        ...
                        $product_name_with_choice =$product->langname();
                        if ($cartItem['variant'] != null) {
                            $product_name_with_choice = $product->langname().' - '.$cartItem['variant'];
                        }
                    @endphp
                    <tr class="cart_item">
